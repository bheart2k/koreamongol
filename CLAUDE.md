...
## 정기 작업 알림 (세션 첫 응답 시 확인 — 필수)

세션에서 **첫 작업 요청에 답할 때**, 아래 표에서 오늘 날짜가 '다음 예정'을 지난 항목이 있으면 답변 첫머리에 한 줄로 알린다: "오늘 하실 작업: ○○". 작업 중간에 반복해서 끼어들지 않는다 (세션당 1회). 해당 작업이 처리되면 이 표의 '마지막 실행'과 '다음 예정'을 즉시 갱신하고, 1회성 작업은 행을 삭제한다.

| 작업 | 주기 | 마지막 실행 | 다음 예정 | 처리 방법 |
|---|---|---|---|---|
| 팩트 재검증 결과 처리 | 매월 (자동 실행: 1일 10:00 작업 스케줄러) | 2026-09-02 (9/1 큐 7건 전부 처리·resolved — 토스는 몽골 송금 미확인으로 사용자 결정에 따라 가이드에서 제거) | 2026-10-01 | `/admin/inbox` 재검증 탭 확인 → 변동 항목 반영. `logs/recheck.log`로 자동 실행 성공 확인, 실패했으면 `/recheck` 수동 실행 |
| 2027년 최저임금·국민연금 요율 갱신 | 1회성 (2027-01-01 적용) | — | 2026-12-28 | jobs.js·tips.js: 시급 ₩10,320→₩10,700, 월 ₩2,156,880→₩2,236,300 (고용노동부 2026-08-05 고시 확정) / 국민연금 근로자 4.75%→5.0% (완료 시 이 행 삭제) |
