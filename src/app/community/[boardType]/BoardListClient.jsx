...
export default function BoardListClient({ boardType, initialData }) {
  ...
  // SWR로 게시글 목록 조회 (초기 데이터는 서버에서 받음)
  ...
  // 초기 데이터 사용: SWR이 아직 로딩 중이고 1페이지이고 검색 없을 때
  const useInitial = isLoading && page === 1 && !searchQuery && initialData;
  const displayPosts = useInitial ? initialData.posts : posts;
  const displayPagination = useInitial ? initialData.pagination : pagination;
  const displayLoading = useInitial ? false : isLoading;
          ...
          {!displayLoading && !error && (
            <div className="flex items-center justify-center gap-2 mt-8">
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => setPage((p) => Math.min(displayPagination.totalPages, p + 1))}
